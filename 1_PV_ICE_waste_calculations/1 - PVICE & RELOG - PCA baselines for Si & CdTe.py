...
interpolatelinearincreasing2035 = True
sparkplot=True
...
for ii in range (len(rawdf.unstack(level=1))):
    ...
    SCEN=SCEN.replace('+', '_')
    filetitle = SCEN+'_cSI_'+PCA +'.csv' #These lines, 2 to 5 will allow to make file names
    subtestfolder = os.path.join(testfolder, 'PCAs_RELOG_Method4') # Create folder of the scenarios
    if not os.path.exists(subtestfolder): # Create folder if it doen's exist
        os.makedirs(subtestfolder)
    filetitle = os.path.join(subtestfolder, filetitle)
    A = rawdf.unstack(level=1).iloc[ii]
    ...
    # Sort Ascending 2022 to 2035 new_Installed_Capacity_[MW]
    # Sort Descending 2035 to 2050 new_Installed_Capacity_[MW]
    if interpolatelinearincreasing2035:
        # LINEAR SLOPE
        yearly2035 = B.iloc[12:26]['new_Installed_Capacity_[MW]'].sum()/105
        yearly2050 = B.iloc[26::]['new_Installed_Capacity_[MW]'].sum()/209
        for kk in range(0, 14):
            B.loc[B.iloc[12+kk].name,'new_Installed_Capacity_[MW]'] = yearly2035*(kk+1)
        for kk in range(0, 15):
            B.loc[B.iloc[26+kk].name, 'new_Installed_Capacity_[MW]' ] = yearly2050*(kk+1)
    # Add other columns
    B = pd.concat([B, baseline.reindex(A.index)], axis=1)

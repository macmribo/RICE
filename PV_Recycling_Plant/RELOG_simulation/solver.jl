...
solution = RELOG.solve("/projects/pvsoiling/RELOG/RELOG_simulation/input/solver/InputPipelineData.json", optimizer=gurobi)
tock()

# Write CSV report showing plant costs, capacities, energy expenditure and
# utilization factors
tick()
RELOG.write_plants_report(solution, "/projects/pvsoiling/RELOG/RELOG_simulation/output/solver/plants.csv")
tock()
...
tick()
RELOG.write_transportation_report(solution, "/projects/pvsoiling/RELOG/RELOG_simulation/output/solver/transportation.csv")
tock()
# Write CSV report showing primary product amounts, locations and marginal costs
tick()
RELOG.write_products_report(solution, "/projects/pvsoiling/RELOG/RELOG_simulation/output/solver/products.csv");
tock()
